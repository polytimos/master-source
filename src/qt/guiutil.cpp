...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("polytimos"))
        return false;
...
{
    // Convert polytimos:// to polytimos:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("polytimos://"))
    {
        uri.replace(0, 12, "polytimos:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Polytimos.lnk";
}
...
{
    return GetAutostartDir() / "polytimos.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Polytimos-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  polytimos-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("Polytimos-Qt"));
